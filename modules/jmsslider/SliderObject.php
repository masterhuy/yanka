...
class SliderObject extends ObjectModel
{
    public $id_slider;
    ...
    public function __construct($id_slide = null, $id_shop = null, $id_lang = null)
    {
        ...
        if ($this->id) {
            $this->lang = Db::getInstance()->getValue("SELECT id_lang FROM "._DB_PREFIX_."jms_slider_lang WHERE id_slider = {$this->id}");
        }
    }
    public function add($auto_date = true, $null_values = false)
    {
        $res = parent::add($auto_date, $null_values);
        $res &= Db::getInstance()->execute("INSERT INTO `"._DB_PREFIX_."jms_slider_lang` VALUES({$this->id}, {$this->lang})");
        return $res;
    }
    public function update($null_values = false)
    {
        $res = parent::update($null_values);
        $res &= Db::getInstance()->execute("UPDATE `"._DB_PREFIX_."jms_slider_lang` SET id_lang = {$this->lang} WHERE id_slider = {$this->id}");
        return $res;
    }
    public function delete()
    {
        ...
        $res&= parent::delete();
        $res &= Db::getInstance()->execute("DELETE FROM `"._DB_PREFIX_."jms_slider_lang` WHERE id_slider = {$this->id}");
        return $res;
    }
    public function getList($active = false, $hook = '', $idlang = 0)
    {
        ...
        if ($hook != '') {
            $sql->innerJoin('jms_slider_hook', 'sh', 's.id_slider = sh.id_slider');
            $sql->innerJoin('jms_hook', 'h', 'sh.id_hook = h.id_hook');
            $sql->where("h.name = '{$hook}'");
        }
        $sql->innerJoin('jms_slider_lang', 'sl', 's.id_slider = sl.id_slider');
        if ($idlang) {
            $sql->where("id_lang = 0 OR id_lang = {$idlang}");
        }
        $sql->orderBy('s.order');
        $rs = Db::getInstance()->executeS($sql);
        return ObjectModel::hydrateCollection('SliderObject', $rs);
    }
    public function getSlides($active = false)
    {
        $slides = new PrestashopCollection('SlideObject');
    ...
    public function duplicateObject()
    {
        ...
        $dupSlider->update();
        Db::getInstance()->execute("INSERT INTO `"._DB_PREFIX_."jms_slider_lang` VALUES({$dupSlider->id}, {$dupSlider->lang})");
        $slides = $this->getSlides();
        foreach ($slides as $slide) {
