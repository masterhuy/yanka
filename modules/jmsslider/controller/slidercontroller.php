...
class Slider
{
    ...
    {
        if ($edit) {
            $slider = new SliderObject((int)Tools::getValue('id_slider'));
        } else {
            $slider = new SliderObject();
        }
        $fields = array(
    ...
    public function renderSlidesList()
    {
        ...
        $this->context->smarty->assign(
            array(
                ...
                'link' => $this->context->link,
                'root_url' => $root_url,
                'img_dir' => $root_url.'/modules/jmsslider/views/img/',
            )
        );
    ...
    public static function getUnhookSliders()
    {
        ...
        $sql->select('*');
        $sql->from('jms_slider');
        $sql->where('id_slider NOT IN (SELECT DISTINCT id_slider FROM '._DB_PREFIX_.'jms_slider_hook)');
