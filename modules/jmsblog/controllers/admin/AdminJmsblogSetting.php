...
class AdminJmsblogSettingController extends ModuleAdminController
{
    ...
    private function _postValidation()
    {
        ...
        if (Tools::isSubmit('JMSBLOG_URL')) {
            if (empty(trim(Tools::getValue('JMSBLOG_URL')))) {
                $errors[] = $this->l('Invalid url');
            }
        }
    ...
    private function _postProcess()
    {
        if (Tools::isSubmit('submitSetting')) {
            ...
            $url = JmsBlogHelper::makeAlias(Tools::getValue('JMSBLOG_URL'));
            $res &= Configuration::updateValue('JMSBLOG_URL', $url);
            ...
            $res &= Configuration::updateValue('JMSBLOG_COMMENT_CAPCHA', Tools::getValue('JMSBLOG_COMMENT_CAPCHA'));
    ...
    public function renderSettingForm()
    {
        ...
        $general_fields = array(
            ...
            array(
                'type' => 'switch',
                'label' => $this->l('Capcha Enable'),
                'name' => 'JMSBLOG_COMMENT_CAPCHA',
                'desc' => $this->l('Enable/Disable Comment Capcha.'),
                'values'    => array(
                    array('id'    => 'active_on','value' => 1,'label' => $this->l('Yes')),
                    array('id'    => 'active_off','value' => 0,'label' => $this->l('No'))
                ),
