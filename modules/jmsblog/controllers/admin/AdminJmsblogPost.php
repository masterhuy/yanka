    ...
    private function _postValidation()
    {
        ...
        if (Tools::isSubmit('changePostStatus')) {
            ...
            if (Tools::isSubmit('category_id')) {
                if (!Validate::isInt(Tools::getValue('category_id')) || Tools::getValue('category_id') <= 0) {
                    $errors[] = $this->l('Invalid category');
                    $this->errors[] = $this->l('Invalid category');
                }
            }
    ...
    private function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitPost')) {
            ...
            foreach ($languages as $language) {
                ...
                $item->alias[$language['id_lang']] = JmsBlogHelper::makeAlias(Tools::getValue('alias_'.$language['id_lang']));
